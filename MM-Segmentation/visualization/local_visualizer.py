...
+----------+---------------+
        ...
        skip_index = [0,1,2,3,4,5,8,9,10,16] # Classes that are excluded from visualization

        mask = np.zeros_like(image, dtype=np.uint8)
        for label, color in zip(labels, colors):
            if label not in skip_index: # Color assessment
                if label == 6 or label == 7:
                    mask[sem_seg[0] == label, :] = [0, 230, 230]
                if label == 17 or label == 15 or label == 14 or label == 13:
                    mask[sem_seg[0] == label, :] = [220, 20, 60]
                if label == 11 or label == 18:
                    mask[sem_seg[0] == label, :] = [0, 0, 142]
